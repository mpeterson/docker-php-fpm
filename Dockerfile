...
ENV LATEST_CACHE 2017.24-10T13:00+02:00
...
RUN apt-get install -y php-fpm php-cli php-memcached php-mbstring\
    php-mysql php-pgsql php-sqlite3 php-gd php-json php-iconv php-simplexml php-zip\
    php-curl php-intl php-imagick php-imap ca-certificates php-apcu
...
RUN sed -i -e '/^memory_limit/c\
memory_limit = 512M' /etc/php/7.2/fpm/php.ini
RUN sed -i -e '/^max_execution_time/c\
max_execution_time = 300' /etc/php/7.2/fpm/php.ini
RUN sed -i -e '/^upload_max_filesize/c\
upload_max_filesize = 16G' /etc/php/7.2/fpm/php.ini
RUN sed -i -e '/^post_max_size/c\
post_max_size = 16G' /etc/php/7.2/fpm/php.ini
RUN sed -i -e '/^output_buffering/c\
output_buffering = 16384' /etc/php/7.2/fpm/php.ini

# Sane defaults php-fpm.conf
RUN sed -i -e '/^;emergency_restart_threshold/c\
emergency_restart_threshold = 10' /etc/php/7.2/fpm/php-fpm.conf

RUN sed -i -e '/^;emergency_restart_interval/c\
emergency_restart_interval = 1m' /etc/php/7.2/fpm/php-fpm.conf

RUN sed -i -e '/^;process_control_timeout/c\
process_control_timeout = 10' /etc/php/7.2/fpm/php-fpm.conf

# Sane defaults www.conf
RUN sed -i -e '/^;pm.max_requests/c\
pm.max_requests = 10000' /etc/php/7.2/fpm/pool.d/www.conf

RUN sed -i -e '/^;request_terminate_timeout/c\
request_terminate_timeout = 300s' /etc/php/7.2/fpm/pool.d/www.conf

RUN sed -i -e 's/^;env/env/g' /etc/php/7.2/fpm/pool.d/www.conf

# Sane defaults OPCache
RUN echo 'opcache.enable=1' \
    >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
RUN echo 'opcache.interned_strings_buffer=8' \
    >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
RUN echo 'opcache.max_accelerated_files=10000' \
     >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
RUN echo 'opcache.memory_consumption=128' \
    >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
RUN echo 'opcache.save_comments=1' \
     >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
RUN echo 'opcache.revalidate_freq=1' \
     >> /etc/php/7.2/fpm/conf.d/05-opcache.ini
...
RUN sed -i -e '/^listen/c\listen = 0.0.0.0:9000' /etc/php/7.2/fpm/pool.d/www.conf
